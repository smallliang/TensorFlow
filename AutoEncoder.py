...
autoencoder = AdditiveGaussianNoiseAutoencoder(n_input=784, n_hidden=200, transfer_function=tf.nn.softplus,
                                               optimizer=tf.train.AdamOptimizer(learning_rate=0.0006), scale=0.01)
...
for epoch in range(training_epochs):
    ...
    plots.append(avg_cost)
print("Total cost: " + str(autoencoder.calc_total_cost(X_test) / int(mnist.test.num_examples)))
ax = plt.subplot()
ax.plot(np.arange(training_epochs), plots, 'r')
